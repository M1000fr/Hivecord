...
export function ConfigProperty(
  ...
  return (target: object, propertyKey: string | symbol) => {
    ...
    constructor.configProperties[propertyKey as string] = options;

    // If the property was initialized with configKey(), attach metadata to it
    const val = (target as Record<string, unknown>)[propertyKey as string];
    if (
      val &&
      typeof val === "object" &&
      ...
      configKeyVal.propertyKey = propertyKey as string;
