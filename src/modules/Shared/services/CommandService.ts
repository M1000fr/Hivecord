...
export class CommandService {
  ...
  private async getI18n(guild: Guild | null): Promise<GuildLanguageContext> {
    const locale = await this.configService.of(guild!, GeneralConfig).Language;
  ...
  async handleAutocomplete(
    ...
    if (method) {
      ...
      await this.invoke(client, interaction, commandInstance, method, langCtx);
    }
  }
  ...
  async execute(
    ...
    if (commandClass.optionRoutes && commandClass.optionRoutes instanceof Map) {
      for (const [optionName, valueMap] of commandClass.optionRoutes) {
        ...
        const route =
          optionValue !== undefined ? valueMap.get(optionValue) : null;
