...
export class ConfigService {
  ...
  of<T>(guild: Guild, configClass: T): ConfigProxy<T> {
    ...
    return new Proxy({} as ConfigProxy<T>, {
      get: (_target, prop) => {
        ...
        // Get default value
        const propertyValue = (configClass as Record<string, unknown>)[prop];
        const defaultValue =
          propertyValue &&
        ...
        switch (options.type) {
          ...
          case EConfigType.ChannelArray:
            return this.channelConfig
              .getList(guild, key)
              .then((v) => (v && v.length > 0 ? v : (defaultValue ?? [])));
  ...
  async getMany(guild: Guild, key: string): Promise<string[]> {
    return this.cache.get(
      ...
      key,
      async () => {
        const configs = await this.configurationRepository.getMany(guild, key);
