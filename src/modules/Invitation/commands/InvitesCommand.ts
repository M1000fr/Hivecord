...
export default class InvitesCommand extends BaseCommand {
    ...
    async run(client: Client, interaction: ChatInputCommandInteraction) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setColor("#0099ff")
            .addFields(
                { name: "Active", value: counts.active.toString(), inline: true },
                { name: "Fake (Left)", value: counts.fake.toString(), inline: true },
                { name: "Total", value: (counts.active + counts.fake).toString(), inline: true }
            )
            .setTimestamp();
