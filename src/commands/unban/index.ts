...
export default class UnbanCommand extends BaseCommand {
    ...
    async run(client: Client, interaction: ChatInputCommandInteraction) {
        ...
        try {
            ...
            await interaction.reply(`User ${user.tag} has been unbanned. Reason: ${reason}`);
        } catch (error: any) {
            await interaction.reply({ content: error.message || "An error occurred while unbanning the user.", flags: [MessageFlags.Ephemeral] });
        }
    }
}
