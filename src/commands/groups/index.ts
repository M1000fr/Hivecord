...
export default class GroupCommand extends BaseCommand {
	...
	private async listGroups(
		...
		const groupList = groups
			.map(
				(g) =>
					`**ID:** ${g.id} | **Name:** ${g.name} | **Role:** <@&${g.roleId}>`,
			)
			.join("\n");
		await interaction.reply({
			content: `**Groups List:**\n${groupList}`,
	...
	private async createGroup(
		...
		try {
			...
			await interaction.reply({
				content: `Group **${group.name}** created with ID **${group.id}** linked to role <@&${role.id}>.`,
				flags: [MessageFlags.Ephemeral],
			});
		} catch (error) {
			console.error(error);
			await interaction.reply({
				content: "An error occurred while creating the group.",
				flags: [MessageFlags.Ephemeral],
			});
		}
	}
	...
	private async updateGroup(
		...
		if (!name && !role) {
			await interaction.reply({
				content:
					"You must provide at least one field to update (name or role).",
				flags: [MessageFlags.Ephemeral],
			});
			return;
		}
		...
		try {
			...
			await interaction.reply({
				content: `Group **${group.id}** updated.`,
				flags: [MessageFlags.Ephemeral],
			});
		} catch (error) {
			console.error(error);
			...
			await interaction.reply({
				content:
					"An error occurred while updating the group. Check if the ID is correct.",
				flags: [MessageFlags.Ephemeral],
			});
		}
	}
	...
	private async deleteGroup(
		...
		try {
			...
			await interaction.reply({
				content:
					"An error occurred while deleting the group. Check if the ID is correct.",
				flags: [MessageFlags.Ephemeral],
			});
		}
	}
