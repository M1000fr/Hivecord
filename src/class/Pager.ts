...
export interface PagerOptions<T> {
    items: T[];
    itemsPerPage: number;
    renderPage: (items: T[], pageIndex: number, totalPages: number) => Promise<{ embeds: EmbedBuilder[], components: any[] }>;
    filter?: (interaction: any) => boolean;
    time?: number;
    onComponent?: (interaction: StringSelectMenuInteraction | ButtonInteraction, collector: InteractionCollector<any>) => Promise<void>;
}
...
export class Pager<T> {
    ...
    private renderPage: (items: T[], pageIndex: number, totalPages: number) => Promise<{ embeds: EmbedBuilder[], components: any[] }>;
    private filter: (interaction: any) => boolean;
    private time: number;
    private onComponent?: (interaction: StringSelectMenuInteraction | ButtonInteraction, collector: InteractionCollector<any>) => Promise<void>;
    ...
    public async start(interaction: RepliableInteraction) {
        ...
        const getPageContent = async () => {
            ...
            const { embeds, components } = await this.renderPage(pageItems, this.currentPage, totalPages);
        ...
        collector.on('collect', async (i) => {
            if (i.customId === 'pager_prev') {
                ...
                if (this.onComponent) {
                    await this.onComponent(i as StringSelectMenuInteraction | ButtonInteraction, collector);
