  ...
  protected async getMainMessage(
    ...
    if (parts[0] === "module_config") {
      if (interaction.isMessageComponent()) {
        if (interaction.message instanceof Message) return interaction.message;
      }
      return null;
    }
    ...
    if (interaction.isMessageComponent()) {
      ...
      if (message instanceof Message) {
        ...
        if (refId) {
          try {
            return (await interaction.channel?.messages.fetch(refId)) || null;
          } catch {
            // Ignore
          }
        }
    ...
    if (interaction.isModalSubmit() && interaction.message instanceof Message) {
      ...
      if (mainMessage) {
        ...
        if (config) {
          try {
            ...
            console.warn("Failed to update config UI message:", error);
          }
        }
      }
  ...
  protected async validateUser(
    ...
    if (interaction.user.id !== userId) {
      await interaction.reply({
        content: "❌ You are not allowed to interact with this component.",
    ...
    if (configContexts?.[selectedProperty]) {
      ...
      const variablesDescription = variables
        .map((v) => {
          ...
          const desc =
            data.descriptionLocalizations?.[lang.locale as Locale] ||
            data.description;
          return `- \`{${v}}\`: ${desc}`;
        })
        .join("\n");
