...
export class DependencyContainer {
  ...
  resolve<T>(
    ...
    if (provider && provider.scope !== "transient") {
      const existing = this.getInstanceByProvider(provider, normalizedModule);
    ...
    if (!provider) {
      ...
      if (target) {
        ...
        dependencySummary = deps.length > 0 ? ` (${deps.join(", ")})` : "";
      }
    ...
    const instance = this.instantiateProvider<T>(provider, normalizedModule);
  ...
  private findProvider(
    ...
    if (normalizedModuleName) {
      ...
      const moduleProviders = this.moduleProviders.get(normalizedModuleName);
      ...
      if (moduleInfo?.options.imports) {
        for (const ImportedModule of moduleInfo.options.imports) {
          ...
          const importedModuleName = importedOptions.name.toLowerCase();
          const exports = this.exportedProviders.get(importedModuleName);
  ...
  private normalizeProvider(
    ...
    if (this.isClassProvider(provider)) {
      return this.createClassProvider(provider.useClass, context, provider);
    }
  ...
  private storeProvider(provider: ResolvedProvider, exports?: ProviderToken[]) {
  ...
  private instantiateProvider<T>(
    ...
    if (this.resolvingStack.has(provider.token)) {
      throw new Error(
        `Circular dependency detected for token '${String(provider.token)}'.`,
      );
    }
    ...
    try {
      ...
      const dependencies = paramTypes.map((paramType, index) => {
        const overrideToken = injectTokens?.[index];
        const tokenToUse = (overrideToken ?? paramType) as ProviderToken;
        return this.resolve(tokenToUse, provider.moduleName ?? moduleName, {
          target: provider.useClass,
          index,
        });
      });
  ...
  private saveInstance<T>(
    ...
    const moduleInstances =
      this.moduleInstances.get(moduleKey) ?? new Map<ProviderToken, unknown>();
  ...
  private getExistingInstance(
    ...
    if (moduleName) {
      const moduleInstance = this.moduleInstances.get(moduleName)?.get(token);
